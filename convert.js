...
function loadConfig(configPath = CONFIG_PATH) {
  ...
  try {
    ...
    if (!customConfig || Array.isArray(customConfig) || typeof customConfig !== "object") {
      console.warn(`警告：${path.basename(configPath)} 必須是 JSON 物件，已改用預設設定。`);
      return DEFAULT_CONFIG;
    }
    ...
    console.warn(`警告：無法讀取 ${path.basename(configPath)}，已改用預設設定。`);
...
function generateHtmlPage(baseName, items, config) {
  const pageTitle = applyTemplate(config.unitBrowserTitleTemplate, { unit: baseName });
  const pageHeading = applyTemplate(config.unitHeadingTemplate, { unit: baseName });
...
function generateIndexPage(manifest, config) {
  const cardsHtml = manifest.map(item => `
    <a href="html/${escapeHtml(item.fileName)}" class="index-card">
      ...
      <h2 class="index-card-title">${escapeHtml(applyTemplate(config.indexCardTitleTemplate, { unit: item.baseName, count: item.count }))}</h2>
      <p class="index-card-desc">${escapeHtml(applyTemplate(config.indexCardDescriptionTemplate, { unit: item.baseName, count: item.count }))}</p>
...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>${escapeHtml(config.indexBrowserTitle)}</title>
  <style>
    :root {
      --bg-color: #f8fafc;
      --card-bg: #ffffff;
...
function main() {
  ...
  for (const file of files) {
    ...
    const htmlContent = generateHtmlPage(baseName, items, config);
  ...
  const indexContent = generateIndexPage(manifest, config);
